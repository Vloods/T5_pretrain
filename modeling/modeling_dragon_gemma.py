...
class DRAGON(nn.Module):
    ...
    def __init__(self, config, args={}, model_name="gemma-2-2b", k=5, n_ntype=4, n_etype= 819 * 2,
        ...
        self.loading_info = json.load(open("gemma_lora.loading_info", 'r'))
...
ModelClass = modeling_gemma2.Gemma2Model
PreTrainedModelClass = modeling_gemma2.Gemma2PreTrainedModel
print("PreTrainedModelClass:", PreTrainedModelClass)


class LMGNN(PreTrainedModelClass):

    def __init__(self, config, args={}, model_name="gemma-2-2b", k=5, n_ntype=4, n_etype=38,
                 n_concept=4335104, concept_dim=200, concept_in_dim=1024, n_attention_head=2,
                 fc_dim=200, n_fc_layer=0, p_emb=0.2, p_gnn=0.2, p_fc=0.2,
                 pretrained_concept_emb=None, freeze_ent_emb=True,
...
class TextKGMessagePassing(ModelClass):
    ...
    def __init__(self, config, args={}, k=5, n_ntype=4, n_etype=38, dropout=0.2, concept_dim=200, ie_dim=200, p_fc=0.2, info_exchange=True, ie_layer_num=1, sep_ie_layers=False):
        ...
        self.encoder.layers = get_peft_model(self.encoder, self.peft_config).layers
...
class Gemma2Config_new(Gemma2Config):
    def __init__(
        self,
        vocab_size=256000,
        hidden_size=3072,
...
class LLaMaGAT(modeling_gemma2.Gemma2PreTrainedModel):

    def __init__(self, config: Gemma2Config, args, k=5, n_ntype=4, n_etype=38, hidden_size=200, dropout=0.2, concept_dim=200, ie_dim=200, p_fc=0.2, info_exchange=True, ie_layer_num=1, sep_ie_layers=False):
        ...
        self.layers = nn.ModuleList(
            [modeling_gemma2.Gemma2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = modeling_gemma2.Gemma2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
#        self._set_default_torch_dtype(torch.bfloat16)
    ...
    def forward(self, hidden_states, attention_mask, special_tokens_mask, head_mask, _X, edge_index, edge_type, _node_type, _node_feature_extra, special_nodes_mask, output_attentions=False, output_hidden_states=True, past_key_values=None, position_ids=None, use_cache=False):
        """
...
def test_LLaMaGAT(device):
    config, unk = Gemma2Config.from_pretrained(
        "google/gemma-2-2b",
        cache_dir=None, return_unused_kwargs=True,
        force_download=False,
        output_hidden_states=True,
    ...
    print(unk)
    test_args = Args()
    model = LLaMaGAT.from_pretrained(config, test_args,  sep_ie_layers=True, torch_dtype=torch.bfloat16).to(device)
    inputs = model.get_fake_inputs(device)
    outputs = model(*inputs)
