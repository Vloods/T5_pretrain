...
def load_data(args, devices, kg, k=None):
    _seed = args.seed
    ...
    if k is not None:
        pattern_s = r'([a-z]+)(.[a-z]+.jsonl)'
        pattern_a = r'([a-z]+)(.[a-z]+.adj.pk)'
        pattern_i = r'([a-z]+)(.txt)'
        to_k = fr'\1_{k}\2'
        train_statements = re.sub(pattern_s, to_k, args.train_statements)
        train_adj = re.sub(pattern_a, to_k, args.train_adj)
        dev_statements  = re.sub(pattern_s, to_k, args.dev_statements)
        dev_adj  = re.sub(pattern_a, to_k, args.dev_adj)
        test_statements = re.sub(pattern_s, to_k, args.test_statements)
        test_adj = re.sub(pattern_a, to_k, args.test_adj)
        if args.dataset == "bookcorpus_splits":
            inhouse_train_qids = re.sub(pattern_i, to_k, args.inhouse_train_qids)
        else:
            inhouse_train_qids = args.inhouse_train_qids
    else:
    ...
    if args.local_rank != -1 and one_process_at_a_time:
        for p_rank in range(args.world_size):
            ...
            dataset = data_utils.DRAGON_DataLoader(args, train_statements, train_adj,
                dev_statements, dev_adj,
                test_statements, test_adj,
                batch_size=args.batch_size, eval_batch_size=args.eval_batch_size,
                device=devices,
                model_name=args.encoder,
                max_node_num=args.max_node_num, max_seq_length=args.max_seq_len,
                is_inhouse=args.inhouse, inhouse_train_qids_path=inhouse_train_qids,
...
#             print("\n\n\n qids, labels, *input_data", qids, labels, *input_data, "\n\n\n")
            bs = labels.size(0)
...
def train(args, resume, has_test_split, devices, kg):
    ...
    model = construct_model(args, kg).to(args.local_rank)
    ...
    if args.lr_schedule == 'fixed':
        ...
        max_steps = int(args.n_epochs * 185 * (dataset.train_size() / args.batch_size))
        try:
    ...
    for epoch_id in trange(0, args.n_epochs, desc="Epoch", disable=args.local_rank not in [-1, 0]): #trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        if last_epoch == epoch_id:
            ...
            start_k = 0
        for k in range(start_k, splits_count):
            if last_epoch > epoch_id: # +1
                time.sleep(1)
                continue
            ...
            if splits_count < 2:
                ...
                dataset = load_data(args, devices, kg, k=k)
            dev_dataloader = dataset.dev()
            if has_test_split:
                test_dataloader = dataset.test()
            model.train()
            ...
            if args.local_rank in [-1, 0]:
                ...
                dev_total_loss = dev_end_loss = dev_mlm_loss = dev_link_loss = dev_pos_link_loss = dev_neg_link_loss = dev_acc = 0.0 # calc_eval_accuracy(args, dev_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path)
...
def evaluate(args, has_test_split, devices, kg):
    ...
    if has_test_split:
        test_dataloader = dataset.test()
    model = construct_model(args, kg, dataset).to(args.local_rank)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dump_graph_cache", default=False, type=utils.bool_flag)
