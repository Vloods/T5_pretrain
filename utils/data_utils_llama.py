...
from transformers import (OpenAIGPTTokenizer, BertTokenizer, BertTokenizerFast, XLNetTokenizer, RobertaTokenizer, RobertaTokenizerFast, LlamaTokenizer, LlamaTokenizerFast, AutoTokenizer)

from preprocess_utils import conceptnet
from utils import utils

MODEL_CLASS_TO_NAME = {
    'llama': ['McGill-NLP/LLM2Vec-Sheared-LLaMA-mntp']
}
...
MODEL_NAME_TO_CLASS[model_name] = 'llama'
    ...
    def _word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        if isinstance(self.tokenizer, (BertTokenizer, BertTokenizerFast)):
            ...
            for (i, token) in enumerate(input_tokens):
                ...
                after_special_tok = False
                effective_num_toks += 1
        elif isinstance(self.tokenizer, (AutoTokenizer, LlamaTokenizer, LlamaTokenizerFast)):
            after_special_tok = False
            for (i, token) in enumerate(input_tokens):
                if token in ["<s>", "</s>", "<unk>"]:
                    after_special_tok = True
...
class DRAGON_DataLoader(object):
    ...
    def load_resources(self, kg):
        # Load the tokenizer
        try:
            tokenizer_class = {'llama': AutoTokenizer, 'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer, 'albert': AlbertTokenizer}.get(self.model_type)
        except:
            tokenizer_class = {'llama': AutoTokenizer, 'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer}.get(self.model_type)
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length, mode='eval'):
        ...
        if use_cache:
            ...
            elif self.model_type in ('bert', 'xlnet', 'roberta', 'albert', 'llama'):
