...
from transformers import (OpenAIGPTTokenizer, BertTokenizer, BertTokenizerFast, XLNetTokenizer, RobertaTokenizer, RobertaTokenizerFast, LlamaTokenizer, LlamaTokenizerFast, AutoTokenizer, GemmaTokenizer, GemmaTokenizerFast)

from preprocess_utils import conceptnet
from utils import utils

MODEL_CLASS_TO_NAME = {
    'gemma': ['google/gemma-2-2b', 'google/gemma-2-9b', 'gemma_lora']
}
...
MODEL_NAME_TO_CLASS[model_name] = 'gemma'
    ...
    def _word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        if isinstance(self.tokenizer, (BertTokenizer, BertTokenizerFast)):
            ...
            for (i, token) in enumerate(input_tokens):
                ...
                after_special_tok = False
                effective_num_toks += 1
        elif isinstance(self.tokenizer, (LlamaTokenizer, LlamaTokenizerFast)):
            after_special_tok = False
            for (i, token) in enumerate(input_tokens):
                if token in ["<s>", "</s>", "<unk>"]:
                    after_special_tok = True
                ...
                after_special_tok = False
                effective_num_toks += 1
        elif isinstance(self.tokenizer, (AutoTokenizer, GemmaTokenizer, GemmaTokenizerFast)):
            after_special_tok = False
            for (i, token) in enumerate(input_tokens):
                if token in ['<unk>', '<bos>', '<eos>', '<pad>']:
                    after_special_tok = True
                    continue
                if len(cand_indexes) >= 1 and (not after_special_tok):
                    cand_indexes[-1].append(i)
...
class DRAGON_DataLoader(object):
    ...
    def load_resources(self, kg):
        # Load the tokenizer
        try:
            tokenizer_class = {'gemma': AutoTokenizer, 'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer, 'albert': AlbertTokenizer}.get(self.model_type)
        except:
            tokenizer_class = {'gemma': AutoTokenizer, 'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer}.get(self.model_type)
        tokenizer = tokenizer_class.from_pretrained(self.model_name)
        if tokenizer.pad_token is None:
            tokenizer.add_special_tokens({'pad_token': '[PAD]'})
            print("toklen: ", len(tokenizer))
        self.tokenizer = tokenizer
        ...
        if kg == "cpnet":
            ...
            with open(cpnet_vocab_path, "r", encoding="utf8") as fin:
                self.id2concept = [w.strip() for w in fin]
            self.concept2id = {w: i for i, w in enumerate(self.id2concept)}
            self.id2relation = list(range(817)) #TODO
        else:
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length, mode='eval'):
        ...
        if use_cache:
            ...
            elif self.model_type in ('bert', 'xlnet', 'roberta', 'albert', 'llama', 'gemma'):
        ...
        if mode == 'train' and self.args.local_rank != -1:
            ...
            example_ids = np.array(example_ids)[_select.tolist()].tolist()
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size):
    ...
    if num_workers <= 1:
        ...
        with Pool(num_workers) as p:
            ress = list(p.starmap(simple_convert_examples_to_features, list(zip(examples_list, label_list_list, max_seq_length_list, tokenizer_list))))
