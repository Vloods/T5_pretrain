...
def concepts2adj(node_ids):
    ...
    n_node = cids.shape[0]
    adj = np.zeros((n_rel, n_node, n_node), dtype=np.uint8)
    sub = cpnet.subgraph(cids)
    maps = {n: i for i, n in enumerate(sub.nodes())}
    for u, v, data in sub.edges(data=True):
        if 0 <= data['rel'] < n_rel:
            adj[data['rel']][maps[u]][maps[v]] = 1
    if n_node == 0:
...
def concepts_to_adj_matrices_2hop_all_pair(data):
    ...
    qa_nodes = set(qc_ids) | set(ac_ids)
    extra_nodes = set()
    sub_qa = cpnet_simple.subgraph(qa_nodes)
    set_qa = {n: set(cpnet_simple[n]) for n in sub_qa.nodes()}
    sub_nodes = list(sub_qa.nodes())
    for i in range(len(sub_nodes)-1):
        for j in range(i+1, len(sub_nodes)):
            extra_nodes |= set_qa[sub_nodes[i]] & set_qa[sub_nodes[j]]
    extra_nodes = extra_nodes - qa_nodes
    schema_graph = sorted(qc_ids) + sorted(ac_ids) + sorted(extra_nodes)
    arange = np.arange(len(schema_graph))
    qmask = arange < len(qc_ids)
