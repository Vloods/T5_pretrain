...
from utils import data_utils_gemma_exp_ft as data_utils
...
def load_data(args, devices, kg, k=None):
    ...
    if k is not None:
        ...
        pattern_i = r'([a-z]+)(.txt)'
        to_k = fr'\1\2_{k}.jsonl'
        to_k_a = fr'\1\2_{k}.adj.pk'
        to_k_i = fr'\1_{k}\2'
        train_statements = re.sub(pattern_s, to_k, args.train_statements)
        train_adj = re.sub(pattern_a, to_k_a, args.train_adj)
        dev_statements  = re.sub(pattern_s, to_k, args.dev_statements)
        dev_adj  = re.sub(pattern_a, to_k_a, args.dev_adj)
        test_statements = re.sub(pattern_s, to_k, args.test_statements)
        test_adj = re.sub(pattern_a, to_k_a, args.test_adj)
        if args.dataset == "bookcorpus_splits":
            inhouse_train_qids = re.sub(pattern_i, to_k_i, args.inhouse_train_qids)
        else:
...
def evaluate(args, has_test_split, devices, kg):
    ...
    bert_or_roberta = model.lmgnn.bert if INHERIT_BERT else model.lmgnn.llama
    bert_or_roberta.resize_token_embeddings(len(dataset.tokenizer))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--load_graph_cache", default=False, type=utils.bool_flag)
